...
int
    zmq5_major = ZMQ_VERSION_MAJOR,
    zmq5_minor = ZMQ_VERSION_MINOR,
    zmq5_patch = ZMQ_VERSION_PATCH;
...
void zmq5_get_event40(zmq_msg_t *msg, int *ev, int *val) {
    zmq_event_t event;
...
void zmq5_get_event41(zmq_msg_t *msg, int *ev, int *val) {
    uint8_t *data = (uint8_t *) zmq_msg_data (msg);
    uint16_t event = *(uint16_t *) (data);
    *ev = (int)event;
    *val = (int)(*(uint32_t *) (data + 2));
}
void *zmq5_memcpy(void *dest, const void *src, size_t n) {
...
func init() {
	...
	if major != 4 {
		initVersionError = fmt.Errorf("Using zmq5 with ZeroMQ major version %d", major)
		return
	}

	v, ok1 := api[[2]int{minor, patch}]
	w, ok2 := api[[2]int{int(C.zmq5_minor), int(C.zmq5_patch)}]
	if v != w || !ok1 || !ok2 {
		if major != int(C.zmq5_major) || minor != int(C.zmq5_minor) || patch != int(C.zmq5_patch) {
			initVersionError =
				fmt.Errorf(
					"zmq5 was compiled with ZeroMQ version %d.%d.%d, but the runtime links with version %d.%d.%d",
					int(C.zmq5_major), int(C.zmq5_minor), int(C.zmq5_patch),
					major, minor, patch)
			return
		}
	}
	...
	var err error
	defaultCtx = &Context{}
	defaultCtx.ctx, err = C.zmq5_ctx_new()
...
func getOption(ctx *Context, o C.int) (int, error) {
	...
	nc, err := C.zmq5_ctx_get(ctx.ctx, o)
	n := int(nc)
	if n < 0 {
		return n, errget(err)
	}
...
func setOption(ctx *Context, o C.int, n int) error {
	...
	i, err := C.zmq5_ctx_set(ctx.ctx, o, C.int(n))
	if int(i) != 0 {
		return errget(err)
	}
	return nil
...
func (ctx *Context) NewSocket(t Type) (soc *Socket, err error) {
	...
	var e error
	for {
		s, e = C.zmq5_socket(ctx.ctx, C.int(t))
...
func (soc *Socket) Connect(endpoint string) error {
	...
	var err error
	for {
		i, err = C.zmq5_connect(soc.soc, s)
		if i == 0 || !soc.ctx.retry(err) {
			break
		}
	}
...
func (soc *Socket) Disconnect(endpoint string) error {
	...
	var err error
	for {
		i, err = C.zmq5_disconnect(soc.soc, s)
		if i == 0 || !soc.ctx.retry(err) {
			break
		}
	}
...
func (soc *Socket) RecvBytes(flags Flag) ([]byte, error) {
	...
	var err error
	for {
		size, err = C.zmq5_msg_recv(&msg, soc.soc, C.int(flags))
	...
	C.zmq5_memcpy(unsafe.Pointer(&data[0]), C.zmq_msg_data(&msg), C.size_t(size))
	return data, nil
}
...
func (soc *Socket) SendBytes(data []byte, flags Flag) (int, error) {
	...
	var err error
	for {
		size, err = C.zmq5_send(soc.soc, unsafe.Pointer(&d[0]), C.size_t(len(data)), C.int(flags))
		if size >= 0 || !soc.ctx.retry(err) {
			break
		}
	}
...
func (soc *Socket) Monitor(addr string, events Event) error {
	...
	if addr == "" {
		...
		var err error
		for {
			i, err = C.zmq5_socket_monitor(soc.soc, nil, C.int(events))
			if i == 0 || !soc.ctx.retry(err) {
				break
			}
		}
	...
	var err error
	for {
		i, err = C.zmq5_socket_monitor(soc.soc, s, C.int(events))
		if i == 0 || !soc.ctx.retry(err) {
			break
		}
	}
...
func (soc *Socket) RecvEvent(flags Flag) (event_type Event, addr string, value int, err error) {
	...
	var e error
	for {
		size, e = C.zmq5_msg_recv(&msg, soc.soc, C.int(flags))
	...
	if minor == 0 {
		C.zmq5_get_event40(&msg, &et, &val)
	} else {
		C.zmq5_get_event41(&msg, &et, &val)
	}
...
func Proxy(frontend, backend, capture *Socket) error {
	...
	var err error
	for {
		_, err = C.zmq5_proxy(frontend.soc, backend.soc, capt)
...
func ProxySteerable(frontend, backend, capture, control *Socket) error {
	...
	var err error
	for {
		i, err = C.zmq5_proxy_steerable(frontend.soc, backend.soc, capt, ctrl)
...
func NewCurveKeypair() (z85_public_key, z85_secret_key string, err error) {
	...
	if i, err := C.zmq5_curve_keypair((*C.char)(unsafe.Pointer(&pubkey[0])), (*C.char)(unsafe.Pointer(&seckey[0]))); i != 0 {
		return "", "", errget(err)
	}
...
func (soc *Socket) RecvBytesWithMetadata(flags Flag, properties ...string) (msg []byte, metadata map[string]string, err error) {
	...
	var size C.int
	for {
		size, err = C.zmq5_msg_recv(&m, soc.soc, C.int(flags))
		if size >= 0 || !soc.ctx.retry(err) {
			break
		}
	}
	...
	data := make([]byte, int(size))
	if size > 0 {
		C.zmq5_memcpy(unsafe.Pointer(&data[0]), C.zmq_msg_data(&m), C.size_t(size))
	}

	if minor > 0 {
		for _, p := range properties {
			ps := C.CString(p)
			s := C.zmq5_msg_gets(&m, ps)
			if s != nil {
